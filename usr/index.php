...
<?php
    ...
    if (isset($_POST['Usr-login'])) {
      ...
      $stmt = $mysqli->prepare("SELECT u_email, u_password, u_id FROM FCT_Users WHERE u_email=? and u_password=? ");//sql to log in user
      $stmt->bind_param('ss', $u_email, $u_password);//bind fetched parameters
      $stmt->execute();//execute bind
      $stmt->bind_result($u_email, $u_password, $u_id);//bind result
      $rs = $stmt->fetch();
      ...
      if ($rs) {//get user logs
          ...
          $log_stmt = $mysqli->prepare("INSERT INTO syslogs(u_id, u_email, u_ip, u_city, u_country) VALUES (?, ?, ?, ?, ?)");
          $log_stmt->bind_param('issss', $uid, $uemail, $ip, $city, $country);
          $log_stmt->execute();
          if ($log_stmt) {
              header("location:/Hall_Booking_WebApplication-master/usr/user-dashboard.php");
          }
      } else {
...
<head>
  ...
  <title>Hall Booking System - User Login</title>
    ...
    <div class="card card-login mx-auto mt-5">
      ...
      <div class="card-body">
        ...
        <form method ="POST">
          ...
          <div class="form-group">
            <div class="form-label-group">
              <input type="password" name="u_password" id="inputPassword" class="form-control"  required="required">
